...
import type { ProjectData, RotationImage } from "../models/rotation_image";
...
export class DataRepository implements Repository {
    ...
    async getImages(objectPath: string): Promise<ProjectData> {
        return this.provider.getImages(objectPath).then((res) => {
            ...
            data.rotationImages.forEach((rotationImage) => {
                rotationImage.image = this.getFullImage(objectPath, rotationImage.name)
                if(data.thumbnails){
                    rotationImage.thumbnail = this.getThumbnail(objectPath, rotationImage.name)
                }
            })
            return data
        })
